...
public class ReplyController {
    ...
    @RequestMapping(value = "/findReplyByCommentId", method = RequestMethod.GET)
    public void findReply(HttpServletResponse response, HttpServletRequest request, Model model) throws IOException {
        response.setCharacterEncoding("UTF-8");
        String commentId  = request.getParameter("commentId");
        System.out.println(commentId);
        Collection<Reply> replies = replyRepository.findByCommentId(commentId);
        Gson gson = new Gson();
        response.getWriter().write(gson.toJson(replies));
    }
